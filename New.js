...
const { Client } = require('pg'); // <-- NEW: PG library for direct DB connection
...
// <-- NEW: Direct PostgreSQL client for DDL operations
...
app.post('/workspace/create', async (req, res) => {
  try {
    ...
    // <-- UPDATED: Insert and get the new workspace ID
    ...
    const sessionsTableName = `ws_sessions_${newWorkspaceId}`; // using a prefix for clarity

    // <-- NEW: Create the new sessions table using the direct DB client
    try {
      ...
      // Decide if you want to fail the entire request here or just log the error.
      // For now, we'll continue, but it's a potential issue.
    }
